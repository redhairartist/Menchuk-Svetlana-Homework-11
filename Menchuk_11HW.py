...
try:
    # Попытка чтения как JSON lines (каждая строка - отдельный объект)
    df = pd.read_json(file_path, lines=True)
except ValueError:
    try:
        # Если не вышло, пробуем считать как обычный JSON файл (массив объектов)
...
if df.empty:
    ...
    # Если данные вложены в колонку 'result', извлекаем их в плоскую таблицу
